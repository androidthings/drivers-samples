...
import com.google.brillo.driver.ssd1306.BitmapHelper;
import com.google.brillo.driver.ssd1306.Ssd1306;
...
public class OledScreenActivity extends Activity {
    ...
    private void drawMovingBmp() {
        Bitmap noPicturesPlz = BitmapFactory.decodeResource(getResources(), R.drawable.nopictures);
        // Move the bmp every few ticks
        if (tick % BMPS == 0) {
            mScreen.clearPixels();
            for (int i = 0; i < 4; i++) {
                if (i == mBmpMod) {
                    int position = i * (int) ((double) noPicturesPlz.getWidth() * 0.9);
                    // Rolls back.
                    if (i == 3) {
                        position = (int) ((double) noPicturesPlz.getWidth() * 0.9);
                    }
                    // Change the x-position but keep y-position at the top.
                    BitmapHelper.setBmpData(mScreen, position, 0, noPicturesPlz, false);
                }
            }
            mBmpMod = (mBmpMod + 1) % 4;
        }
